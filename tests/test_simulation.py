...
# ---------------------------------------------------------------------------
# Phase 19 Task 2: compute_ticker_consensus tests
# ---------------------------------------------------------------------------
...
def _make_persona(agent_id: str, bracket: BracketType, influence_weight_base: float = 0.7) -> AgentPersona:
    return AgentPersona(
        id=agent_id,
        name=agent_id,
        bracket=bracket,
        ...
        temperature=0.5,
        system_prompt="test",
        influence_weight_base=influence_weight_base,
    )


def _make_bracket(bracket_type: BracketType, display_name: str) -> BracketConfig:
...
def test_compute_ticker_consensus_majority() -> None:
    """3 agents (2 BUY, 1 SELL) -> majority_signal=BUY, majority_pct=2/3 fraction."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "buy"),
        _make_agent_with_ticker_decision("a2", "quants", "buy", "AAPL", "buy"),
        _make_agent_with_ticker_decision("a3", "degens", "sell", "AAPL", "sell"),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
        _make_persona("a2", BracketType.QUANTS),
        _make_persona("a3", BracketType.DEGENS),
    ]
    brackets = [_make_bracket(BracketType.QUANTS, "Quants"), _make_bracket(BracketType.DEGENS, "Degens")]
    weights = {"a1": 1.0, "a2": 1.0, "a3": 1.0}

    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 1
    assert result[0].ticker == "AAPL"
    assert result[0].majority_signal == "BUY"
    assert result[0].majority_pct == pytest.approx(2 / 3, abs=0.01)  # fraction, NOT 66.7


def test_compute_ticker_consensus_weighted() -> None:
    """2 agents with different weights -> correct weighted_signal and weighted_score."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "buy", confidence=0.9),
        _make_agent_with_ticker_decision("a2", "quants", "sell", "AAPL", "sell", confidence=0.7),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
        _make_persona("a2", BracketType.QUANTS),
    ]
    brackets = [_make_bracket(BracketType.QUANTS, "Quants")]
    weights = {"a1": 0.8, "a2": 0.3}

    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert result[0].weighted_signal == "BUY"
    # BUY sum = 0.9 * 0.8 = 0.72, SELL sum = 0.7 * 0.3 = 0.21, total = 0.93
    assert result[0].weighted_score == pytest.approx(0.72 / (0.72 + 0.21), abs=0.01)


def test_compute_ticker_consensus_bracket_scope_uses_direction() -> None:
    """CRITICAL: bracket breakdown uses td.direction, NOT decision.signal."""
    from alphaswarm.simulation import compute_ticker_consensus

    # Agent1: global signal=BUY but TickerDecision direction=SELL for AAPL
    # Agent2: global signal=SELL but TickerDecision direction=BUY for AAPL
    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "sell"),
        _make_agent_with_ticker_decision("a2", "quants", "sell", "AAPL", "buy"),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
        _make_persona("a2", BracketType.QUANTS),
    ...
    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 1
    quants_bd = result[0].bracket_breakdown[0]
    # Bracket breakdown must use td.direction: a1=SELL, a2=BUY -> 1 sell, 1 buy
    assert quants_bd.sell_count == 1
    assert quants_bd.buy_count == 1


def test_compute_ticker_consensus_excludes_parse_error() -> None:
    """Agent with PARSE_ERROR signal is excluded from count."""
    from alphaswarm.simulation import compute_ticker_consensus
    ...
    decisions = [
        ...
        ("a2", AgentDecision(
            ...
            confidence=0.0,
            ticker_decisions=[TickerDecision(ticker="AAPL", direction=SignalType.SELL)],
        )),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
    ...
    assert len(result) == 1
    assert result[0].majority_signal == "BUY"
    assert result[0].majority_pct == pytest.approx(1.0)  # 1/1 = 100%


def test_compute_ticker_consensus_excludes_empty_ticker_decisions() -> None:
    """Agent with empty ticker_decisions list is excluded."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "buy"),
        ("a2", AgentDecision(signal=SignalType.BUY, confidence=0.8, ticker_decisions=[])),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
        _make_persona("a2", BracketType.QUANTS),
    ...
    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 1
    # Only a1 contributes, so total votes = 1
    assert result[0].majority_pct == pytest.approx(1.0)


def test_compute_ticker_consensus_empty_result() -> None:
    """When all agents are PARSE_ERROR, returns empty tuple."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        ("a1", AgentDecision(
            signal=SignalType.PARSE_ERROR, confidence=0.0,
            ticker_decisions=[TickerDecision(ticker="AAPL", direction=SignalType.BUY)],
        )),
    ]
    personas = [_make_persona("a1", BracketType.QUANTS)]
    brackets = [_make_bracket(BracketType.QUANTS, "Quants")]
    weights = {"a1": 1.0}
...
def test_compute_ticker_consensus_multiple_tickers() -> None:
    """Agents with decisions for AAPL and TSLA -> 2 entries sorted alphabetically."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        ("a1", AgentDecision(
            signal=SignalType.BUY, confidence=0.8, sentiment=0.0, rationale="test",
            ticker_decisions=[
                TickerDecision(ticker="TSLA", direction=SignalType.SELL),
                TickerDecision(ticker="AAPL", direction=SignalType.BUY),
            ],
        )),
        ("a2", AgentDecision(
            signal=SignalType.SELL, confidence=0.7, sentiment=0.0, rationale="test",
            ticker_decisions=[
                TickerDecision(ticker="AAPL", direction=SignalType.SELL),
                TickerDecision(ticker="TSLA", direction=SignalType.BUY),
            ],
        )),
    ]
    personas = [
    ...
    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 2
    assert result[0].ticker == "AAPL"  # sorted alphabetically
    assert result[1].ticker == "TSLA"


def test_compute_ticker_consensus_influence_fallback() -> None:
    """Agent not in influence_weights uses persona.influence_weight_base as fallback."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "buy", confidence=0.9),
    ]
    personas = [_make_persona("a1", BracketType.QUANTS, influence_weight_base=0.5)]
    brackets = [_make_bracket(BracketType.QUANTS, "Quants")]
    weights: dict[str, float] = {}  # a1 NOT in weights -> fallback to 0.5

    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 1
    # weighted_score = (0.9 * 0.5) / (0.9 * 0.5) = 1.0 (only one vote)
    assert result[0].weighted_score == pytest.approx(1.0)


def test_compute_ticker_consensus_tie_break_determinism() -> None:
    """Equal votes BUY vs HOLD -> BUY wins (deterministic tie-break)."""
    from alphaswarm.simulation import compute_ticker_consensus

    decisions = [
        _make_agent_with_ticker_decision("a1", "quants", "buy", "AAPL", "buy", confidence=0.8),
        _make_agent_with_ticker_decision("a2", "quants", "hold", "AAPL", "hold", confidence=0.8),
    ]
    personas = [
        _make_persona("a1", BracketType.QUANTS),
        _make_persona("a2", BracketType.QUANTS),
    ...
    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert result[0].majority_signal == "BUY"  # BUY wins tie over HOLD
    assert result[0].weighted_signal == "BUY"  # BUY wins tie over HOLD


def test_compute_ticker_consensus_ticker_normalization() -> None:
    """Lowercase ticker 'aapl' normalized to 'AAPL'. No duplicate rows."""
    from alphaswarm.simulation import compute_ticker_consensus
    ...
    result = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    assert len(result) == 1  # No duplicate rows
    assert result[0].ticker == "AAPL"  # Normalized to uppercase


async def test_compute_ticker_consensus_simulation_wiring() -> None:
    """Integration test: set_ticker_consensus called 3 times, snapshot reflects latest round."""
    from alphaswarm.simulation import compute_ticker_consensus
    ...
    # Round 1
    r1 = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=1)
    await store.set_ticker_consensus(r1)
    snap1 = store.snapshot()
    assert len(snap1.ticker_consensus) == 1
    assert snap1.ticker_consensus[0].round_num == 1

    # Round 2
    r2 = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=2)
    await store.set_ticker_consensus(r2)
    snap2 = store.snapshot()
    assert snap2.ticker_consensus[0].round_num == 2

    # Round 3
    r3 = compute_ticker_consensus(decisions, personas, brackets, weights, round_num=3)
    await store.set_ticker_consensus(r3)
    snap3 = store.snapshot()
    assert snap3.ticker_consensus[0].round_num == 3
