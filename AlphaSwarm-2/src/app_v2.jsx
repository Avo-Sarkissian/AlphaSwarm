// Main app — committed to Quant Terminal direction
...
function Tooltip({ info }) {
  ...
  return (
    <div className="tooltip" style={{ left: x + 14, top: y + 14 }}>
      ...
      <div className="t-row"><span>Peer reads</span><strong>{12 + (agent.index % 18)}</strong></div>
...
function App() {
  const [onboarding, setOnboarding] = useState(false); // disabled during dev
  ...
  const agents = useMemo(
    () => window.AS_DATA.buildAgents(round, state.sigMix),
    [round, state.sigMix.buy, state.sigMix.sell, state.sigMix.hold]
  );
  const summaries = useMemo(() => window.AS_DATA.bracketSummaries(agents), [agents]);
  ...
  const onAgentClick = (agent) => setUi(u => ({...u, interviewV2Agent: agent}));
    ...
    <div className="app" data-dir="a" data-density={state.density} data-screen-label="01 Dashboard">
      ...
      <div className="topbar">
        ...
        <div style={{flex:1, minWidth:200, position:'relative', display:'flex', alignItems:'center', gap:6}}>
          ...
          {ui.seedSuggestOpen && (
            <SeedSuggest
              onPick={(seed) => setState(s => ({...s, seed}))}
              onClose={() => setUi(u => ({...u, seedSuggestOpen: false}))}
            />
          )}
        </div>
        ...
        <ModelStatus running={state.running} phase={state.phase === 'done' ? 'done' : state.phase} tps={tps} slots={slots} />
        ...
        <div style={{position:'relative'}}>
          ...
          {ui.overflowOpen && (
            <div style={{
              ...
              {[
                { label:'What-if Compare',       icon:'graph',   action: u => ({...u, whatIfOpen: true,      overflowOpen: false}) },
                { label:'Multi-seed Synthesis',  icon:'doc',     action: u => ({...u, multiSeedOpen: true,   overflowOpen: false}) },
                { label:'Data Sources',          icon:'search',  action: u => ({...u, dataSourcesOpen: true, overflowOpen: false}) },
                { label:'Portfolio Stress Test', icon:'brief',   action: u => ({...u, stressTestOpen: true,  overflowOpen: false}) },
                { label:'Advisory (v6.0)',        icon:'brief',   action: u => ({...u, advisoryOpen: true,    overflowOpen: false}) },
                { label:'Customize Brackets',    icon:'settings',action: u => ({...u, bracketCustomOpen: true,overflowOpen: false}) },
              ].map(item => (
      ...
      <div className="main">
        ...
        <div className="col col-left">
          <div className="panel" style={{flex: '0 0 auto'}}>
            ...
            <div className="panel-body">
              <BracketList summaries={summaries} onClick={(s) => setUi(u => ({...u, bracketDeepDive: s}))} />
            </div>
          </div>
        ...
        <div className="col col-right">
          <div className="panel">
            ...
            <div className="panel-body">
              <RationaleFeed
                rationales={window.AS_DATA.RATIONALES}
      ...
      {/* Modals */}
      {ui.interviewV2Agent && (
        <InterviewV2 agent={ui.interviewV2Agent} onClose={() => setUi(u => ({...u, interviewV2Agent: null}))} />
      )}
      {ui.bracketDeepDive && (
        <BracketDeepDive
      ...
      {ui.whatIfOpen && <WhatIfCompare onClose={() => setUi(u => ({...u, whatIfOpen: false}))} />}
      {ui.watchOpen && <WatchMode onClose={() => setUi(u => ({...u, watchOpen: false}))} onOpenReport={() => setUi(u => ({...u, watchOpen:false, reportOpen:true}))} />}
      {ui.multiSeedOpen && <MultiSeedSynthesis onClose={() => setUi(u => ({...u, multiSeedOpen: false}))} />}
      {ui.stressTestOpen && <PortfolioStressTest onClose={() => setUi(u => ({...u, stressTestOpen: false}))} />}
      {ui.bracketCustomOpen && <BracketCustomizer onClose={() => setUi(u => ({...u, bracketCustomOpen: false}))} />}
      {ui.reportOpen && <ReportModalV2 onClose={() => setUi(u => ({...u, reportOpen: false}))} />}
      {ui.advisoryOpen && <AdvisoryV2 onClose={() => setUi(u => ({...u, advisoryOpen: false}))} />}
      {ui.dataSourcesOpen && <DataSourcesTakeover onClose={() => setUi(u => ({...u, dataSourcesOpen: false}))} />}
      {ui.historyOpen && (
        <CycleHistory
      ...
      {onboarding && (
        <Onboarding onComplete={(seed, model) => {
          if (seed) setState(s => ({...s, seed, demoState: 'seeding'}));
          setOnboarding(false);
        }} />
      )}
    </div>
