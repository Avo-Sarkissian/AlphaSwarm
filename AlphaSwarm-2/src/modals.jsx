...
function ShockDrawer({ onClose, onInject }) {
  const [text, setText] = useStateM('DOJ files formal antitrust complaint against Apple–Anthropic deal');
  const presets = [
...
function AdvisoryModal({ onClose }) {
...
function ReplayBar({ cycle, round, onRound, onExit }) {
...
function CyclePickerModal({ onPick, onClose }) {
  const { REPLAY_CYCLES } = window.AS_DATA;
  return (
    <div className="modal-backdrop" onClick={onClose}>
...
window.InterviewModal = InterviewModal;
window.ReportModal = ReportModal;
window.ShockDrawer = ShockDrawer;
window.AdvisoryModal = AdvisoryModal;
window.ReplayBar = ReplayBar;
window.CyclePickerModal = CyclePickerModal;
