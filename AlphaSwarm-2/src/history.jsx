...
function CycleHistory({ onClose, onOpenReport }) {
  const [query, setQuery] = useStateH('');
  const [filter, setFilter] = useStateH('all'); // all | starred | today
  const [selected, setSelected] = useStateH(new Set());
  const [compareOpen, setCompareOpen] = useStateH(false);
...
Object.assign(window, { CycleHistory, CompareModal });
